#!/bin/bash
os_code_name=$(cat /etc/lsb-release | awk -F '=' 'NR==2 { print $2 }')
...
# font
if test -z "$(whitch fc-list)"; then
    sudo apt install -y -qq fontconfig
...
echo "font have been config."
# fishshell
sudo apt-get install -y -qq fish
echo /usr/local/bin/fish | sudo tee -a /etc/shells
chsh -s /usr/local/bin/fish 
echo "fish shell have been installed."
# alacritty
...
echo "alacritty have been installed."
